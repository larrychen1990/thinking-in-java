...
public class ClassScanner {
	...
	private MultiStringMap classMap = new MultiStringMap(),
	...
	public void checkClassNames() {
		...
		while (files.hasMoreElements()) {
			...
			for (int i = 0; i < cls.size(); i++) {
				String className = (String) cls.elementAt(i);
			...
			for (int i = 0; i < ids.size(); i++) {
				...
				if (!classes.contains(id)) {
					...
					if (Character.isUpperCase(id.charAt(0))) {
						if (reportSet.indexOf(file + id) == -1) { // Not
																	// reported
																	// yet
							reportSet.addElement(file + id);
							System.out.println("Ident capitalization error in:"
									+ file + ", ident: " + id);
						}
					}
				}
	...
	public static void main(String[] args) {
		...
		if (old.exists()) {
			try {
				...
				System.err.println("Could not open " + old + " for reading");
				System.exit(1);
			}
		}
		if (args.length == 1) {
