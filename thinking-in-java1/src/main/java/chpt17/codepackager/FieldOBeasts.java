...
class Beast {
	...
	public void step() {
		...
		if (seen.size() > 0) {
			...
			while (e.hasMoreElements()) {
				...
				float bearing = aBeast.bearingFromPointAlongAxis(x, y,
						currentDirection);
	...
	public float bearingFromPointAlongAxis(int originX, int originY, float axis) {
		...
		try {
			double bearingInRadians = Math.atan((this.y - originY)
					/ (this.x - originX));
...
public class FieldOBeasts extends Applet implements Runnable {
	...
	static int halfFieldOfView = (int) ((fieldOfView * 360) / (2 * Math.PI)),
			xExtent = 0, yExtent = 0, numBeasts = 50, maxSpeed = 20; // Pixels/second
	...
	Vector makeBeastVector(int quantity, boolean uniqueColors) {
		...
		for (int i = 0; i < quantity; i++) {
			...
			newBeasts.addElement(new Beast(this, x, y, direction, speed,
					new Color(r, g, b)));
		}
		return newBeasts;
	}
	...
	public Vector beastListInSector(Beast viewer) {
		...
		while (e.hasMoreElements()) {
			...
			if (aBeast != viewer) {
				...
				float bearing = aBeast.bearingFromPointAlongAxis(v.x, v.y,
