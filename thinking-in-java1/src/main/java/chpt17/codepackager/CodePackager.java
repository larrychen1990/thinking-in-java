...
class IO {
	...
	static DataOutputStream dosOpen(File f) {
		...
		try {
			in = new DataOutputStream(new BufferedOutputStream(
...
class SourceCodeFile {
	public static final String startMarker = "//:", // Start of source file
			...
			System.getProperty("line.separator"), filesep = // System's file
															// path separator
	...
	public SourceCodeFile(String firstLine, BufferedReader in) {
		...
		try {
			while ((s = in.readLine()) != null) {
				...
				if (s.startsWith(endMarker) || s.startsWith(endMarker2)) {
					foundEndMarker = true;
					break;
				}
			}
			if (!foundEndMarker)
				Pr.error("End marker not found before EOF");
	...
	public SourceCodeFile(BufferedReader pFile) {
		try {
			...
			if (!s.startsWith(packMarker))
				Pr.error("Can't find " + packMarker + " in " + s);
...
class DirMap {
	...
	public void writePackedFile(String fname) {
		...
		try {
			packed.writeBytes("###Old Separator:" + SourceCodeFile.filesep
					+ "###\n");
		} catch (IOException e) {
	...
	public void write() {
		...
		while (e.hasMoreElements()) {
			...
			IO.close(IO.dosOpen(new File(new File(rootpath, dir), Integer
					.toString(v.size()) + ".files")));
		}
	}
}

public class CodePackager {
	private static final String usageString = "usage: java CodePackager packedFileName"
	...
	private static void extractPackedFile(String[] args) {
		...
		if (s.indexOf("###Old Separator:") != -1) {
			String oldsep = s.substring("###Old Separator:".length());
